...
outputdir="/scratch/abraha1/gsel_/gsel_vec/test/snp_list_output"
analysis_name="testlist"
...
# CLUMP list
OutObj = clump_snp_list(snps_list_file, outputdir, analysis_name, thous_gen_file, clump_r2_threshold=0.9)

# MATCH
ldbuds_r2_threshold="friends_ld05"
match_OutObj =match_snps(OutObj.get('lead_snp_ld_pairs_r2'), num_control_sets, ldbuds_r2_threshold, snpsnap_db_file, outputdir, analysis_name)

# GET LD SNPS FOR CONTROL SNPS
csnps_file = match_OutObj.get('matched_snps_file')
ldexp_OutObj = get_ldsnps_for_control_snps(csnps_file, thous_gen_file, outputdir, analysis_name, clump_r2_threshold)
...
ldexp_match_OutObj = ld_expand_all_control_snps(lead_snps_ld_counts_file, gwas_snps_r2_file, matched_file, control_ld_dir, outputdir, analysis_name)
# lead_ld_counts_file = lead_snps_ld_counts_file
...
combined_summary_df, mean_sd_ldscore_df = check_ld_expanded_sets(snpsnap_db_file, ld_expanded_control_sets_file , lead_snps_ld_counts_file, ld_expanded_control_sets_r2_file, ldbuds_r2_threshold, outputdir, analysis_name)
