...
def pooled_overlap(long_df):
    ...
    count_df = (
        long_df.groupby(["lead_snp"])
        .size()
        .reset_index()
        .rename(columns={0: "num_snps"})
    )
    missing_df = (
        long_df.groupby(["lead_snp"])["is_na"]
        .sum()
        .reset_index()
        .rename(columns={"is_na": "num_missing_anno"})
    )

    anno_coverage_df = pd.merge(count_df, missing_df, on="lead_snp", how="outer")
    anno_coverage_df["prop_coverage"] = 100 - (
        (anno_coverage_df["num_missing_anno"] / anno_coverage_df["num_snps"]) * 100
    ).round(1)
...
def per_set_overlap(long_df):
    ...
    count_df = (
        long_df.loc[long_df["set"] != "ld_snp"]
        .groupby(["lead_snp", "set"])
        .size()
        .reset_index()
        .rename(columns={0: "num_snps"})
    )
    ...
    anno_coverage_df = pd.merge(
        count_df, missing_df, on=["lead_snp", "set"], how="outer"
    )
    anno_coverage_df["prop_coverage"] = 100 - (
        (anno_coverage_df["num_missing_anno"] / anno_coverage_df["num_snps"]) * 100
    ).round(1)
    ...
    # exclude gwas snps
    by_control_summary_df = (
        anno_coverage_df.groupby(["lead_snp"])
        .agg({"prop_coverage": ["mean", "std", "count"]})
        .reset_index()
    )
...
def grouby_stat(control_long_df, gwas_long_df, by):
    ...
    if by == "mean":
        gwas_df = (
            gwas_long_df.groupby(["lead_snp", "set"]).mean().unstack().reset_index()
        )
        cntrl_df = (
            control_long_df.groupby(["lead_snp", "set"]).mean().unstack().reset_index()
        )
    elif by == "median":
        gwas_df = (
            gwas_long_df.groupby(["lead_snp", "set"]).median().unstack().reset_index()
        )
        cntrl_df = (
            control_long_df.groupby(["lead_snp", "set"])
            .median()
            .unstack()
            .reset_index()
        )
        ...
        gwas_df = (
            gwas_long_df.groupby(["lead_snp", "set"]).max().unstack().reset_index()
        )
        cntrl_df = (
            control_long_df.groupby(["lead_snp", "set"]).max().unstack().reset_index()
        )
    elif by == "min":
        gwas_df = (
            gwas_long_df.groupby(["lead_snp", "set"]).min().unstack().reset_index()
        )
        cntrl_df = (
            control_long_df.groupby(["lead_snp", "set"]).min().unstack().reset_index()
        )
...
def get_summary_by_lead_snp(long_df, summary_by):
    ...
    control_long_df = long_df.loc[
        long_df["set"] != "ld_snp", ["lead_snp", "set", "anno"]
    ].copy()
    gwas_long_df = long_df.loc[
        long_df["set"] == "ld_snp", ["lead_snp", "set", "anno"]
    ].copy()

    assert summary_by in [
        "mean",
        "median",
        "max",
        "min",
    ], "summary_by values is not valid"
...
def intersect_annotation(
    ...
    long_df = pd.melt(
        match_df,
        id_vars=["lead_snp"],
        value_vars=["ld_snp"] + control_cols,
        var_name="set",
        value_name="snp",
    )
    ...
    summary_by_pval_df, summary_by_z_score_df = create_pval_zscore_df(
...
def set_up_outputs(OutputObj, anno_label_list, summary_type):
    ...
    for anno_label in anno_label_list:
        ...
        OutputObj.add_output(
            "{}_pooled_overlap_output".format(anno_label),
            "pooled_{}_overlap.tsv".format(anno_label),
            custom_root=anno_root,
        )
        OutputObj.add_output(
            "{}_by_control_set_overlap_output".format(anno_label),
            "by_control_set_{}_overlap.tsv".format(anno_label),
            custom_root=anno_root,
        )
        OutputObj.add_output(
            "{}_{}_output".format(anno_label, summary_type),
            "{}_{}_per_lead_snp.tsv".format(summary_type, anno_label),
            custom_root=anno_root,
        )
        ...
        OutputObj.add_output(
            "{}_pvalue".format(anno_label),
            "pvalue_{}_per_lead_snp.tsv".format(anno_label),
            custom_root=anno_root,
        )
        OutputObj.add_output(
            "{}_zscore".format(anno_label),
            "zscore_{}_per_lead_snp.tsv".format(anno_label),
            custom_root=anno_root,
        )
        OutputObj.add_output(
            "{}_annotation_values".format(anno_label),
            "{}_annotation_values.tsv".format(anno_label),
            custom_root=anno_root,
        )

        OutputObj.add_output(
            "{}_enrichment_summary".format(anno_label),
            "{}_enrichment_summary.tsv".format(anno_label),
            custom_root=anno_root,
        )
...
def create_pval_zscore_df(summary_df, anno_label, two_sided_bool_dict):
    ...
    z_score_df["z_score"] = (
        z_score_df.lead_snp_anno - z_score_df.mean_controls
    ) / z_score_df.std_controls
    z_score_df["n_controls"] = z_score_df.loc[:, control_cols].apply(
        lambda x: np.sum(~x.isnull()), axis=1
    )
    z_score_df = z_score_df.loc[
        :, ["lead_snp", "lead_snp_anno", "z_score", "mean_controls", "std_controls"]
    ].copy()
    ...
    # calc p-value
    partial_pval = partial(
        calc_pval_and_summary,
        control_cols=control_cols,
        two_tailed_bool=two_tailed_bool,
    )
    ...
    if no_na_pval_df.shape[0] == 0:
        na_df = pd.DataFrame(
            {
                "lead_snp": na_pval_lead_snps,
                "pvalue": np.nan,
                "test_type": np.nan,
                "lead_snp_anno": np.nan,
                "input_percentile": np.nan,
                "num_control_snps": np.nan,
    ...
    hypo_reject_bh, pval_corrected_bh, _, _ = multipletests(
...
def calc_pval_and_summary(values, control_cols, two_tailed_bool):
    ...
    if np.isnan(input_snp_value) or (len(na_removed_control_values) == 0):
        ...
        summary = [
            ...
            input_snp_value,
            np.nan,
            len(na_removed_control_values),
        ]

        return summary

    # A percentileofscore of 80% means that 80% of values are less than or equal to the provided score.
    input_percentile = stats.percentileofscore(
        na_removed_control_values, input_snp_value, kind="weak"
    )
...
def intersect_all_annotations(
    anno_path_dict, two_sided_bool_dict, summary_type, matched_file, output_root
):
    ...
    anno_label_paths_pairs = [
        (anno_label, anno_path) for anno_label, anno_path in anno_path_dict.items()
    ]
    ...
    for anno_result in intersect_ouputs:
        ...
        combined_df = pd.merge(
            pd.merge(
                by_control_set_overlap.loc[:, ["lead_snp", "mean_prop", "std_prop"]],
                summary_by_df.loc[:, ["lead_snp", "lead_snp_anno"]],
                on="lead_snp",
                how="outer",
            ),
            summary_by_pval_df.loc[
                :,
                [
                    "lead_snp",
                    "pvalue",
                    "reject_h0_benj_hoch",
                    "corrected_pval_benj_hoch",
                ],
            ],
            on="lead_snp",
            how="outer",
        ...
        combined_df.to_csv(
            OutObj.get("{}_enrichment_summary".format(anno_label)),
            sep="\t",
            index=False,
        )
        summary_by_df.to_csv(
            OutObj.get("{}_{}_output".format(anno_label, summary_type)),
            sep="\t",
            index=False,
        )
        pooled_overlap_df.to_csv(
            OutObj.get("{}_pooled_overlap_output".format(anno_label)),
            sep="\t",
            index=False,
        )
        by_control_set_overlap.to_csv(
            OutObj.get("{}_by_control_set_overlap_output".format(anno_label)),
            sep="\t",
            index=False,
        )

        # add the type of summary stat used for selection measures ...
        summary_by_pval_df.to_csv(
            OutObj.get("{}_pvalue".format(anno_label)),
            sep="\t",
            index=False,
            na_rep="NaN",
        )
        summary_by_pval_df["summary_type_over_input_region"] = summary_type
        summary_by_zscore_df.to_csv(
            OutObj.get("{}_zscore".format(anno_label)),
            sep="\t",
            index=False,
            na_rep="NaN",
        )
        ...
        annotation_long_df.to_csv(
            OutObj.get("{}_annotation_values".format(anno_label)),
            sep="\t",
            index=False,
            na_rep="NaN",
        )
